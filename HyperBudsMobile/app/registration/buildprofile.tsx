...
// Local fallback avatar image
const defaultAvatar = require('../../assets/images/avatar.png');

// Optional backend (used only to read username/avatar on load; no errors shown if missing)
...
export default function Profile() {
  ...
  // Load username (+ initial avatar) from API if configured, else fallback to Firebase user info
  useEffect(() => {
    let isMounted = true;
    ...
    const loadProfile = async () => {
      try {
        ...
        if (API_BASE) {
          ...
          setAvatarUri(data.avatarUrl || user.photoURL || undefined);
        } else {
          if (!isMounted) return;
  ...
  // Match buildprofile.tsx: pick image and show it locally (no upload)
  ...
  return (
    <SafeAreaView style={styles.safe}>
      <ScrollView contentContainerStyle={styles.container}>
        ...
        {/* Username & Role */}
        <Text style={styles.username}>{loadingProfile ? 'Loading…' : displayHandle}</Text>
        ...
        <Text style={styles.bio}>
          Short bio entry will go here but for now placeholder text to simulate a user's bio
          description.
        </Text>
